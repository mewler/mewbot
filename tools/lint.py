...
class Annotation:
    ...
    def __str__(self) -> str:
        ...
        return f"::{self.level} file={self.file},line={self.line},col={self.col},title={self.title}::{mess}"
...
class LintToolchain:
    ...
    def run_tool(self, name: str, *args: str) -> subprocess.CompletedProcess:
        arg_list = list(args)
    ...
    def lint_black(self) -> Generator[Annotation, None, None]:
        ...
        yield from self.lint_black_errors(result)
        yield from self.lint_black_diffs(result)

    def lint_black_errors(self, result):
    ...
    def lint_black_diffs(self, result):
        file = ""
        line = 0
        buffer = ""
